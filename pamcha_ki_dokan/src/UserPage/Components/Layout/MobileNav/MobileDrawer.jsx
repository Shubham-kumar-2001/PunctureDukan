...
const MobileDrawer = (props) => {
  ...
  const serviceDrawerHandler = () => {
    setServiceDrawer(!serviceDrawer);
  };
    ...
    <div className="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform  bg-white w-64 dark:bg-gray-800">
      ...
      <div className="py-4 overflow-y-auto">
        <ul className="space-y-2 font-medium">
          {authCtx.user_id && (
            <>
              {userMobileNav.map((userNav, index) => (
                <MobilenavButton
                  handler={userDrawerHandler}
                  name={userNav.name}
                  username={authCtx.user_id}
                  fabIcon={userNav.fabIcon}
                  onClick={props.onClick}
                  key={index}
                  list={userNav.list}
                  Drawer={userDrawer}
              ...
              {credentialMobilenav.map((cred, index) => (
                <MobilenavButton
                  handler={credentialDrawerHandler}
                  name={cred.name}
                  fabIcon={cred.fabIcon}
                  onClick={props.onClick}
                  key={index}
                  list={cred.list}
          ...
          {serviceMobileNav.map((service, index) => (
            <MobilenavButton
              handler={serviceDrawerHandler}
              name={service.name}
