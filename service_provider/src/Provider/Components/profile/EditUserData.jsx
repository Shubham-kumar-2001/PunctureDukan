import React from "react";
import TextInput from "../../../UI/InputForm";
import RadioInput from "../../../UI/RadioInput";
import ButtonForm from "../../../UI/buttonForm";
import { useHttpClient } from "../../../Hooks/http-hook";
import { toast } from "react-toastify";
import { useState } from "react";
const EditUserData = (props) => {
  const { isLoading, sendRequest } = useHttpClient();
  const handleError = (err) =>
    toast.error(err, {
      position: "top-right",
    });
  const handleSuccess = (msg) =>
    toast.success(msg, {
      position: "top-right",
    });
  const { username, _id, created, ...restData } = props.userData;
  const [values, setValues] = useState({ ...restData });
  console.log(values, "values");

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setValues({
      ...values,
      [name]: value,
    });
  };
  const userDataUpdateHandler = async (event) => {
    try {
      event.preventDefault();
      const data = await sendRequest(
        `https://puncturedukan.onrender.com/api/puncturedukan/serviceprovider/updateuser`,
        "PUT",
        JSON.stringify({ ...values }),
        {
          "Content-Type": "application/json",
        }
      );
      const { success, message } = data;
      if (success) {
        handleSuccess(message);
      } else {
        handleError(message);
      }
    } catch (err) {
      handleError(err.message);
    }
  };
  return (
    <div className="space-y-6 flex flex-col flex-wrap mb-[2rem] ">
      <button
        onClick={props.handleShowDetail}
        type="button"
        class="sm:!hidden text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 absolute top-[9.4%] left-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white"
      >
        <svg
          class="w-5 h-5"
          aria-hidden="true"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 14 14"
        >
          <path
            stroke="currentColor"
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke-width="2"
            d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"
          />
        </svg>
      </button>
      <form
        onSubmit={userDataUpdateHandler}
        className="flex flex-wrap flex-col overflow-hidden 
        relative w-[100%] bg-white space-y-6 mx-auto"
      >
        <div className="flex flex-wrap items-start mt-[2rem] md:mt-0">
          <span className="text-[30px] font-semibold ml-[2rem] text-zinc-600">
            Personal Information
          </span>

          <div className="grid gid-cols-1 lg:grid-cols-2 gap-4 w-[95%] mx-auto mt-[2rem]">
            <TextInput
              label="FirstName"
              input={{
                type: "text",
                name: "firstName",
                id: "firstName",
              }}
              onChange={handleInputChange}
              defaultValue={values.firstname}
            />

            <TextInput
              label="LastName"
              input={{
                type: "text",
                name: "lastName",
                id: "lastname",
              }}
              onChange={handleInputChange}
              defaultValue={values.lastname}
            />
          </div>
        </div>
        <div className="flex flex-col items-start ml-[2rem] space-y-4">
          <span className="block text-[23px] font-medium text-zinc-500">
            Your Gender
          </span>
          <div className="flex items-start">
            <RadioInput
              label="Male"
              value="Male"
              required={true}
              onChange={handleInputChange}
              checked={values.gender === "Male" ? true : false}
            />
            <RadioInput
              label="Female"
              value="Female"
              onChange={handleInputChange}
              checked={values.gender === "Female" ? true : false}
            />
            <RadioInput
              label="Other"
              value="Other"
              onChange={handleInputChange}
              checked={values.gender === "Other" ? true : false}
            />
          </div>
        </div>
        <div className="flex flex-col items-start ml-[2rem] space-y-4">
          <span className="block text-[23px] font-medium text-zinc-500">
            Email Address
          </span>
          <div className="flex items-start w-[95%] md:w-[60%]">
            <TextInput
              label="Email"
              input={{
                type: "email",
                name: "email",
              }}
              onChange={handleInputChange}
              defaultValue={values.email}
            />
          </div>
        </div>
        <div className="flex flex-col items-start ml-[2rem] space-y-4">
          <span className="block text-[23px] font-medium text-zinc-500">
            Phone Number
          </span>
          <div className="flex items-start w-[95%] md:w-[60%]">
            <TextInput
              label="Mobile No"
              input={{
                type: "text",
                name: "phone",
                maxLength: "10",
                id: "phone",
              }}
              onChange={handleInputChange}
              defaultValue={values.mobilenumber}
            />
          </div>
        </div>
        <div className="flex justify-center mx-auto space-x-6 w-[95%] ">
          <ButtonForm
            type="submit"
            buttonContent={
              isLoading ? (
                <div role="status">
                  <svg
                    aria-hidden="true"
                    class="inline w-5 h-5 mr-2 text-white animate-spin dark:text-gray-600 fill-blue-900"
                    viewBox="0 0 100 101"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                      fill="currentColor"
                    />
                    <path
                      d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                      fill="currentFill"
                    />
                  </svg>
                  <span class=" text-white text-[15px] ml-[0.7rem]">
                    Loading...
                  </span>
                </div>
              ) : (
                "Update"
              )
            }
            className={`text-[20px] w-full text-white bg-green-700 hover:green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800
               `}
          />
        </div>
      </form>
    </div>
  );
};

export default EditUserData;
