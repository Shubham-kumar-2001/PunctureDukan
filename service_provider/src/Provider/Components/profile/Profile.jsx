...
const Profile = () => {
  ...
  const { isLoading: isLoadingLogout, sendRequest: logoutSendRequest } =
    useHttpClient();
  const { isLoading: isAvatarLoading, sendRequest: avatarSendRequest } =
    useHttpClient();
    ...
    const fetchUsers = async () => {
      try {
        const responseData = await sendRequest(
          `https://puncturedukan.onrender.com/api/puncturedukan/serviceprovider/user`
        );
  ...
  const onUpload = async (event) => {
    try {
      ...
      const data = await avatarSendRequest(
        `${process.env.REACT_APP_UPDATE_PROVIDER}`,
        "PUT",
                ...
                <div className="flex  h-[100px] w-[100px]  mb-3">
                  ...
                  {isAvatarLoading && (
                    <div role="status">
                      <svg
                        aria-hidden="true"
                        class="inline w-8 h-8 mr-2 text-center mt-[1rem] text-gray-200 animate-spin dark:text-gray-600 fill-green-600"
                        viewBox="0 0 100 101"
                        fill="none"
